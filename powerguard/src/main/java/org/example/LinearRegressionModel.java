...
public class LinearRegressionModel {
    ...
    private void ensureHeaderConsistency() {
        if (this.datasetHeader != null && this.datasetHeader.classIndex() == -1) {
            this.datasetHeader.setClassIndex(this.datasetHeader.numAttributes() - 1);
        }
    }
    ...
    public double predict(double[] features) throws Exception {
        ...
        int expectedFeatureCount = datasetHeader.numAttributes() - 1;
        if (features == null || features.length != expectedFeatureCount) {
            throw new IllegalArgumentException(
                    "Invalid feature count. Expected " + expectedFeatureCount + " but got " +
                    (features == null ? 0 : features.length)
            );
        }

        double[] instanceValues = new double[datasetHeader.numAttributes()];
        System.arraycopy(features, 0, instanceValues, 0, expectedFeatureCount);
        instanceValues[datasetHeader.classIndex()] = 0.0;
    ...
    public int getExpectedFeatureCount() {
        return datasetHeader == null ? 0 : datasetHeader.numAttributes() - 1;
    }

    public String[] getFeatureOrder() {
        if (datasetHeader == null) {
            throw new IllegalStateException("Dataset header not initialized.");
        }

        int classIndex = datasetHeader.classIndex();
        String[] names = new String[datasetHeader.numAttributes() - 1];
        int pointer = 0;
        for (int i = 0; i < datasetHeader.numAttributes(); i++) {
            if (i == classIndex) continue;
            names[pointer++] = datasetHeader.attribute(i).name();
        }
        return names;
    }
